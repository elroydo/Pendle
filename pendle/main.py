...
# GUI jazz
# # ball moves left to right repeatedly
def move_ball():
    global function_ID, start_time, direction # Access global variables

    elapsed_time = int(time.time() - start_time) # Timer
    if direction == 1:
        session_screen.move(ball, -12, 0) # Move ball 10 pixels to the left
        if session_screen.coords(ball)[0] < 30: # Check coordinates
            direction = 0 # Left
    else:
        session_screen.move(ball, 12, 0) # Move ball 10 pixels to the right
        if session_screen.coords(ball)[2] > 770:
            direction = 1 # Right
    ...
    if elapsed_time >= 300: # 5 minute timer
        ...
        function_ID = session_screen.after(10, move_ball) # Update canvas every 10ms
...
def show_session_screen(event=None):
    ...
    move_ball() # Call function to move ball
...
def show_help_screen(event=None):
    ...
    help_screen.create_text(400, 230, text="Pendle uses a swinging ball, inspired by Eye-Movement Desensitisation and Reprocessing (EMDR) therapy, to help you process your experiences while feeling calm and relaxed.", 
...
ball = session_screen.create_oval(x - radius, y - radius, x + radius, y + radius, fill="black") # Create ball
