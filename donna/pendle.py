...
from tkinter import BOTH, FLAT, PhotoImage, Tk, Canvas, Button
...
class Physiology(threading.Thread):
    ...
    def saveData(self, data):
        ...
        if not os.path.isfile(filepath): #check if file exists
            with open(filepath, mode='a', newline='') as csv_file:
                ...
                writer.writerow(['bpm', 'brpm', 'emotions']) # headers
                for metric in data:
    ...
    def run(self):
        ...
        global stopThread, monitoring, elapsedTime
        ...
        while True:
            ...
            cv.putText(frame, f'BPM: {bpmBuffer.mean()}', (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1) #display bpm
            cv.putText(frame, f'BRPM: {bpmBuffer.mean()//4}', (10, 60), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1) #display brpm
            cv.putText(frame, f'Emotion: {str(dominantEmotion)}', (10, 90), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1) #display emotion in frame
        ...
        #zip up data to make it iterable
        data = zip(heartRate, respirationRate, emotions)
...
def showCanvas1(event=None):
    ...
    canvas1.create_text(400, 700, text="End your session anytime by pressing on the screen", font=fontSubHeading, fill="black")
    helpButton.pack()
    helpButton.place(x=750, y=750) #button position on canvas
    
def showCanvas2(event=None):
    global startTime, monitoring, monitor, stopThread
    stopThread = False
    if monitoring: #check if monitor is up and running
        startTime = time.time() #get current time
    monitor = Physiology(daemon=True)
...
#help button
helpIcon = PhotoImage(file='./assets/images/question.png')
restartIcon = PhotoImage(file='./assets/images/restart.png')
helpButton = Button(canvas1, text="Help", command=showHelpCanvas, image=helpIcon, relief=FLAT) #create button
restartButton = Button(canvas4, text="Restart", command=showCanvas1, image=restartIcon, relief=FLAT) #create button
